  ...
  if (start < articleNum) {
    for (var i = start; i < endFor; i++) {
      ...
      <div class="cf-article">
        <a class="cf-article-title" href="${item.link}" target="_blank" rel="noopener nofollow" data-title="${
        item.title
      }">${item.title}</a>
        ...
        <div class="cf-article-avatar no-lightbox flink-item-icon">
          <a onclick="openMeShow(event)" data-link="${
            item.link
          }" class="" target="_blank" rel="noopener nofollow" href="javascript:;"><img class="cf-img-avatar avatar" src="${
        item.avatar
      }" alt="avatar" onerror="this.src='${fcdata.error_img}'; this.onerror = null;"><span class="cf-article-author">${
        item.author
      }</span></a>
  ...
  var showHtml = `
      <div class="cf-overshow">
        <div class="cf-overshow-head">
          <img class="cf-img-avatar avatar" src="${userinfo.avatar}" alt="avatar" onerror="this.src='${fcdata.error_img}'; this.onerror = null;">
  ...
  fetch(url)
    ...
    .then(json => {
      ...
      if (json.message === "not found") {
        btf.snackbarShow("抱歉, 没有找到他的列表呜呜呜。");
        noClick = "ok";
      } else {
        document.getElementById("cf-overlay").className = "cf-show-now";
